"""
This is a little graph cut segmentation example. 
It requires PyMaxflow.
"""
...
data_dir = '/home/eric/Desktop/career/branding/onlinePortfolio/GraphCutSegmentation'
im_original = Image.open( os.path.join(data_dir, 'original2_v1.jpg') )
im_marked = Image.open( os.path.join(data_dir, 'marked2_v2.jpg') )
...
fg_mask = (arr_marked[:,:,0] > fg_color[0]-eps) * 
          (arr_marked[:,:,1] < fg_color[1]+eps) * 
          (arr_marked[:,:,2] > fg_color[2]-eps)
bg_mask = (arr_marked[:,:,0] < bg_color[0]-eps) * 
          (arr_marked[:,:,1] > bg_color[0]+eps) * 
          (arr_marked[:,:,2] < bg_color[0]-eps)
...
E_data_fg = 1-exp(-( 
                (R-mu_fg[0])**2/(2*sigma_fg[0]**2) + 
                (G-mu_fg[1])**2/(2*sigma_fg[1]**2) + 
                (B-mu_fg[2])**2/(2*sigma_fg[2]**2) )
                )
...
if do_plots:
    ...
    #plt.colorbar()
    plt.axis('off')
    plt.savefig(os.path.join(data_dir, 'fg_energy.jpg'), bbox_inches='tight')

[mu_bg, sigma_bg] = GetMeanAndStd(arr_original,bg_mask)
E_data_bg = 1-exp(-( 
                (R-mu_bg[0])**2/(2*sigma_bg[0]**2) + 
                (G-mu_bg[1])**2/(2*sigma_bg[1]**2) + 
                (B-mu_bg[2])**2/(2*sigma_bg[2]**2) )
                )
if do_plots:
    plt.figure()
    ...
    #plt.colorbar()
    plt.axis('off')
    plt.savefig(os.path.join(data_dir, 'bg_energy.jpg'), bbox_inches='tight')
...
if do_plots:
    ...
    plt.axis('off')
    plt.savefig(os.path.join(data_dir, 'final_segmentation.jpg'), bbox_inches='tight')
